...
public class Uni2DSpriteAnimation
{
	...
	public bool Play(string a_rClipName)
	{
		int iClipIndex = GetClipIndexByName( a_rClipName );
		if( iClipIndex == -1 )
		{
			return false;
		}
	...
	public void Play( int a_iClipIndex )
	{
	...
	public int GetClipIndexByName(string a_rClipName)
	{
		...
		{
			for( int iClipIndex = 0, iClipCount = this.ClipCount; iClipIndex < iClipCount; ++iClipIndex )
			{
				if( m_rAnimationClips[ iClipIndex ] != null && m_rAnimationClips[ iClipIndex ].name == a_rClipName )
				{
					return iClipIndex;
				}
			}
	...
	{
		return 0 <= a_iClipIndex && a_iClipIndex < this.ClipCount ? m_rAnimationClips[ a_iClipIndex ] : null;
	}
