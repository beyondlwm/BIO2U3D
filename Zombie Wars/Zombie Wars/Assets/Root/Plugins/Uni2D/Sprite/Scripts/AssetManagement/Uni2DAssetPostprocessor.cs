...
public class Uni2DAssetPostprocessor : AssetPostprocessor {
	...
	// HACK/WORKAROUND: when calling GenerateTextureImportGUID and setting asset labels, Unity will re-import the assets.
	// This import is useless and can lead to an infinite loop. This list keeps a ref to processed imported assets
	// and break the loop when re-imported assets are processed again.
	private static HashSet<string> ms_oImportedTexturesWithUpdatedUni2DGUIDs = new HashSet<string>( );
